...
const char * const BITCOIN_CONF_FILENAME = "mbm.conf";
const char * const BITCOIN_PID_FILENAME = "mbmd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "mbm" is a composite category enabling all Multi Bit Master-related debug output
                if(ptrCategory->count(std::string("mbm"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mbmCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mbmCore
    // Mac: ~/Library/Application Support/mbmCore
    // Unix: ~/.mbmcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "mbmCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/mbmCore";
#else
    // Unix
    return pathRet / ".mbmcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty mbm.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override mbm.conf
